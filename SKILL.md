...
**模板：`assets/chat_template_v11.html`**（v8 大动画系统 + v11 纯聊天条白带布局 + 入场动画 10 种随机 + 表情包，鸿哥 2026-08-12 定稿）
...
| **布局（鸿哥 2026-08-11 定稿）** | **聊天框居中，占总画面 40-60%**；**上方 0-40% 是「重点信息区」**——放放大的聊天重点文字（hook/textzoom）和元素动画（表情包/光效），**聊天框内只呈现聊天内容**，两者分离 |
| **UI 生命周期（鸿哥 2026-08-11 定稿）** | **开场 3-5 秒**显示完整微信界面（状态栏+导航栏+输入栏，让人看出"这是微信"）→ **之后 UI 全部淡出隐藏**（`#phone background: transparent`，输入栏 display:none），画面只留「头像+气泡」消息条，干净利落 |
| **消息保留策略（鸿哥 2026-08-11 定稿）** | 画面**保留最近 4-6 条消息**（KEEP_MSGS=6），更早的淡出；**新消息在底部弹出，旧消息往上滑**（每条消息元素自身 translateY(-scrollY)，容器不动），模拟微信滚动记录 |
...
| 字幕（鸿哥 2026-08-11 定稿） | **根据说唱内容生成字幕，每条 ≤15 字**；说唱内容=聊天文字时，字幕就是聊天文字。v6 模板自带字幕层（#subtitle）：屏幕底部大字白字黑描边，自动跟随当前激活消息（activeEl）的文字（取首行、截断 15 字）。**字幕跟随消息时间轴 = 跟随 whisper 对齐后的人声**（消息 start_ms 已按 whisper 段起点排布，见第 2.5 步），人声唱到哪句字幕正好显示哪句（卡拉 OK 级同步） |
| 情绪动画（鸿哥 2026-08-11 定稿） | **聊天框根据消息情绪做对应动画**（元素可选 `emotion` 字段，默认 normal）：`surprise`惊讶→聊天框裂开抖动（scaleX 震 + 中缝裂缝）、`excited`兴奋→微笑弯曲（下边框变弧）+轻微抖动、`angry`生气→左右晃动+倾斜+红色调、`sad`难过→下沉+压扁+灰调、`laugh`大笑→左右摇摆+弹跳、`zoom`放大强调→整体轻微放大。动画强度随时间衰减，自动复原。**⚠️ 只用 scale + filter 实现，禁止 rotate/translate**（会把 #phone 布局带飞，消息移位/消失） |
| 聊天框动态大小（鸿哥 2026-08-11 定稿） | **聊天框大小不固定不变**：①全局轻微"呼吸"（幅度 0.8%、周期 ~4s 缓慢放大缩小，画面不死板）；②**每条消息进入时轻微放大脉冲**（scale 1→1.03→1，前 450ms，跟说唱节奏同拍）；③情绪动画优先级最高。**不是每时每刻剧烈变**，保持自然节奏感 |
| 气泡尺寸（鸿哥 2026-08-11 定稿） | 聊天条整体**放大 1.5 倍**：气泡文字 50px、头像 126px 方形圆角、气泡间距加大、画面保留 5-6 条。**气泡文本不换行**（脚本生成时 `text.replace(/\n/g,'')` 合并单行） |
| z-index（鸿哥 2026-08-11 定稿） | `#messages` 显式 `z-index:0`，确保导航栏 `#nav-bar`(z-index 50) 永远盖在消息上面——**消息上滑经过导航栏时被裁切隐藏，不能遮住对方昵称栏** |
...
- **时长 = 内容自然长度（鸿哥 2026-08-11 确认）**：20-35 条元素（不是 8-12 条！），**不为凑 60/90/120 秒而人为拉长 duration/加停顿**；5 幕结构（钩子→铺垫→冲突→反转→爆点）；共鸣点优先；最后一条炸（"散会！/拜拜！"级 hook）
- 剧本 `duration_ms` 总和要覆盖 BGM 全长，别留大段空档
- **说唱词 = 完整聊天文字**：BGM 歌词 = 全部对白逐句按顺序排列，不许跳过或改写（敏感词处理见下）
- **消息文案写作规范（鸿哥 2026-08-11 定稿）**：
...
### ⭐ 第 0 步：对话审阅确认（鸿哥 2026-08-11 定稿，必做！）

**鸿哥给主题后，第一步【只生成完整对话内容】给他审阅，等他确认后再往下跑。** 流程：

1. 按「聊天内容生成规则」生成**完整微信对话**（每人 12-15 条、来回 1-3 条、1-2 个梗、每条 ≤12 字适合说唱）
2. 用聊天窗口格式**在回复里展示完整对话**（标注角色：老板/我，气泡方向），**不要直接生成视频**
3. 明确问鸿哥：「对话确认后我就跑视频」——等他说"确认/可以/OK"再进第 1 步
4. 鸿哥要改：按反馈调整对话，重新展示，再等确认

> 铁律：**未经鸿哥确认对话，禁止继续生成 BGM/渲染/合成**。鸿哥说"直接开干/直接生成视频"时才跳过确认。
...
export PATH="/Users/fudasu/.workbuddy/binaries/node/workspace/node_modules/ffmpeg-static:$PATH"
HTTPS_PROXY= HTTP_PROXY= https_proxy= http_proxy= \
  /Users/fudasu/.workbuddy/binaries/python/envs/default/bin/mlx_whisper bgm.mp3 \
...
### 第 7 步：生成封面图（**必做**，跑完视频自动出，不等鸿哥说）

**跑完视频直接生成封面**（鸿哥 2026-08-11 定稿：给主题 → 跑视频 → 直接出封面 → 聊天框交付）。用 ImageGen 生成 **16:9 横版 + 9:16 竖版** 两张，输出到 `out/cover/`。

> **⚠️ 鸿哥 2026-08-14 定稿换风格**：封面从原来的"白底 iPad"风格 → 改用 **Codex 黑底红蓝金融/科技风格**（Typography-led + Editorial-grid）。原因：Codex 那张封面鸿哥认可度极高，有抖音短视频需要的爆点+数据感+冲击力。**以后所有 chat-rap-video 主题封面默认按这个风格做**，用 poster-style-transfer skill 套用。

#### Codex 封面风格规范（2026-08-14 鸿哥定稿）

| 要素 | 要求 |
|------|------|
| **Archetype** | Typography-led + Editorial-grid（标题是 hero，文字堆叠做信息架构） |
| **背景** | **纯黑 #000** + 红/蓝渐变曲线（K 线 / 折线 / 箭头）+ 暗红/暗蓝柱状图 |
| **排版层级** | **5 段必须严格保留**（从上到下、左对齐堆叠在画面左上到中下） |
| ① 顶部标签胶囊 | 青色描边，约 12-15% 宽，**左对齐左上角**，内容形如 `VS [对方] \| [时间戳]` |
| ② 主标题 | **超大粗体白色**，约 1/4 高度，1-2 行（如"DeepSeek 涨价了"） |
| ③ 副标题 | **次大粗体红色**，约 1/6 高度，1-2 行（如"输出贵了 4.5 倍"） |
| ④ 重点问句/概念胶囊 | **青色描边 + 红底**，约 30-40% 宽，居中或居左（如"峰谷价来了"） |
| ⑤ 底部小标语 | **白色 + 关键点强调青色**，约 1/12 高度，1 行（如"V4 Pro 性能猛，但钱包先破防"） |
| **字体** | 无衬线粗体；中文粗黑体；英文 sans-serif 粗体；weight ≥ 700 |
| **配色** | 黑 #000（背景）+ 白 #fff（主标题）+ 红 #e60012（副标题/胶囊底）+ 青 #00d4ff（边框/强调文字） |
| **背景元素** | 金融/科技/数据感：K 线、时钟、芯片/网络节点、柱状图、人物剪影（焦虑/操作姿态）——按主题替换 |
| **人物处理** | 暗调剪影或低饱和度，不抢文字，仅装饰 |
| **整体氛围** | 数据感 + 科技感 + 红蓝对比，**不允许纯白底** |

#### Codex 风格 ImageGen prompt 模板

直接调用 `poster-style-transfer` skill（直接生成模式），让它用下方 variable template 套用 Codex 风格：

```
Codex 风格海报，[16:9 横版 | 9:16 竖版]，[新主题]。

设计规则（必须严格遵守）：
1. 纯黑 #000 背景
2. 排版层级 5 段（从上到下、左对齐堆叠）：
   a. 顶部青色描边胶囊："VS [对方] | [时间戳]"，字号最小，左上角
   b. 主标题：超大粗体白色，约画面 1/4 高度，1-2 行，[主标题]
   c. 副标题：次大粗体红色，约画面 1/6 高度，1-2 行，[副标题]
   d. 重点问句/概念胶囊：青色描边 + 红底，居中或居左，[重点问句]
   e. 底部小标语：白色 + 关键点强调青色，[标语]
3. 字体：无衬线粗体；中文粗黑体；英文 sans-serif 粗体；weight ≥ 700
4. 文字位置：左对齐，文字堆叠在画面左上到中下
5. 文字位置、字号、字体不变，只换文字内容和背景图
6. 背景元素（金融/科技/数据感）：K 线/折线、时钟、芯片/网络、柱状图、人物剪影——按新主题调整
7. 配色：黑底 + 红色重点 + 青色边框/文字 + 白色主标题
8. 整体氛围：金融数据感 + 科技感 + 红蓝对比

参考两张 Codex 封面（一张 16:9 横版，一张 9:16 竖版，已存档到 skill assets）：
- 横版参考图：`assets/cover_references/codex-2026-08-14/codex_16x9.png`
- 竖版参考图：`assets/cover_references/codex-2026-08-14/codex_9x16.png`

输入图片仅作为风格和设计系统参考，不要复制具体人物、构图、文字内容、品牌标识。
```

**变量填法**：
- `[新主题]` = 视频主题（如"DeepSeek 涨价"、"便宜主播 vs 抠门老板"、"亲戚 vs 大龄未婚"）
- `[VS 对方]` = 对话另一方称呼（如"AI 圈老铁"、"抠门老板"、"催婚亲戚"）
- `[时间戳]` = 对话开场时间戳（如"23:40"），可在剧本里抄
- `[主标题]` = 视频主钩子（如"DeepSeek 涨价了"、"便宜主播"）
- `[副标题]` = 数据/补充（如"输出贵了 4.5 倍"、"月薪只有 5000"）
- `[重点问句]` = 核心冲突（如"峰谷价来了"、"99 一场童叟无欺"）
- `[标语]` = 底部金句（如"V4 Pro 性能猛，但钱包先破防"、"我虽然便宜 但我不傻"）

#### 调用方式（生成时）

```bash
# 用 poster-style-transfer skill 的直接生成模式
# 输入：Codex 参考图 + variable template + 7 个变量值
# 输出：16:9 横 + 9:16 竖 各 1 张
```

**注意**：
- 每张约 5-10 credits，两张共 10-20 credits
- 生成前告诉鸿哥总费用预估
- 中文气泡/文字 AI 还原可能不准 → 用 poster-style-transfer 的 fidelity-rubric 自审一次，文字乱码立即重生成
- Codex 参考图必须传两张（横+竖）让 poster-style-transfer 看清排版在两种比例下的行为

#### 鸿哥未来如何覆盖风格

如果鸿哥之后又改了封面风格偏好：
1. 把新参考图存到 `assets/cover_references/<日期>/`
2. 更新本节表格和 prompt 模板
3. 在 changelog 记一笔（旧风格废止时间）

### 第 8 步：聊天框交付（**必做**，一步到位）

**① 保存到桌面（必做，鸿哥 2026-08-12/13 定规矩）**：视频和封面**必须**复制到 `~/Desktop/ai/抖音发视频/<日期>/`（日期格式 YYYY-MM-DD，如 `2026-08-13`）：
...
**② 聊天框交付 = present_files 一次传全部**（鸿哥 2026-08-11 定稿：最后在聊天框交付，聊天框里可直接预览）：
- 传入：`out/final.mp4`（视频，聊天框自动播放）+ `out/cover/` 下 16:9、9:16 两张封面
- 顺序：视频放第一（优先展示），封面随后
- 文字说明：**桌面保存路径** + 文件路径 + 视频时长 + 封面要点 + credits 消耗

### 第 9 步：CloudStudio 部署手机预览（可选，鸿哥要看手机效果时）
...
> 10. 第 8 步：**存桌面 `~/Desktop/ai/抖音发视频/<日期>/`（必做）** + present_files 聊天框交付；鸿哥要手机预览才第 9 步部署

## 执行注意（给 agent）

- **花钱边界**：Flova CLI 需会员（月费真钱）+ credits（1-5/首音乐）。提交生成前必须向鸿哥复述 prompt/模型/预估 credits 再确认。ImageGen 生成头像/表情包每张约 5-10 credits，也要先说。本地 ffmpeg/截图/渲染不花钱。
...
- **v4/v5/v6/v7/v8 演进经验**（改模板时参考）：
  ...
  - **v7（2026-08-12 鸿哥指定参考抖音 https://v.douyin.com/SlIGgVlDbBk/「汽水AI写歌教程」）升级动画风格**：
  ...
  - **v8（2026-08-12 鸿哥"动画要有艺术感、幅度要偶尔大"）大动画系统**：
  ...
  - **保留**（鸿哥已稳定认可）：聊天框居中 + 上方重点区 + UI 淡出 + KEEP_MSGS + 字幕 + 情绪动画 + from 字段铁律 + whisper 对齐
...
- **交付前自审清单（必做，2026-08-11 定稿）**：
  ...
  4. 全部通过再交付
- **用户反馈"旧画面"先怀疑缓存（2026-08-11 实测）**：鸿哥截图说"整个视频都有蓝线/灰底聊天框/绿色底部"，但抽帧验证新视频完全干净 → 结论是他手机/浏览器**缓存的旧版本**。处理：`unpublish` 旧部署 → 重新 `deploy`（换新链接）→ 明确告诉用户"你看到的旧画面是缓存，请用新链接 + 强制刷新"。另外**截图里蓝椭圆+斜线常是 macOS 自带截屏工具画的标注**（圈问题点用），不是视频内容，先抽帧确认再判断。

## v11（2026-08-12 鸿哥指定参考抖音 https://v.douyin.com/AIAF8BFqhT4/「王总又作妖了·律师的真实日常」）纯聊天条白带布局
...
## v11b：表情包 + 入场动画随机化（2026-08-12 鸿哥要求）
...
**鸿哥要求**：表情不要只局限 emoji，要猫猫狗狗等可爱贴纸、线条人等多样化风格。
...
## v11d：表情素材库扩充 v2——高频梗+去水印（2026-08-13 鸿哥"多贴微信表情图"）

**鸿哥反馈**：表情图还是少了，**下次记得多贴一些微信表情图**。这条不重新生成，扩充素材库 + 写规则到 skill。
...
DeferExecuteTool({
  params: {
    ...
    output_dir: '/Users/fudasu/WorkBuddy/2026-08-11-16-17-52/out/assets_<unique>',
    quality: 'high',
    size: '1024x1024',
